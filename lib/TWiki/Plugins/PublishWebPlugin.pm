...
sub initialize {
    ...
    $publishWeb = TWiki::Func::getPluginPreferencesValue("PUBLISHWEBNAME")
      || "Publish";
    $publishSkin = TWiki::Func::getPluginPreferencesValue("PUBLISHSKIN")
      || "print";
...
sub publishTopic {
    ...
    $tmpl =~
      s/(^|[\(\s])([A-Z][A-Za-z0-9]*)\.([A-Z]+[a-z]+[A-Za-z0-9])/$1<nop>$3/go;
    ...
    $tmpl =~
      s/($pubUrl)\/([^'" ]+)/&fixAndCopyAttachments($1, $2, $pubDir )/geo;
    ...
    if ( $action eq "breadcrumb" ) {
        ...
        if ( $topicName eq "all" ) {
            ...
            if ( TWiki::Func::topicExists( $publishWeb, $topicName ) ) {
                if ( publishTopic( $publishWeb, $topicName ) ) {
                    $text =
                      "PUBLISHWEB: Published topic [[$publishWeb.$topicName]]";
                }
                else {
                    $text =
"PUBLISHWEB error: Topic [[$publishWeb.$topicName]] not published";
                }
            }
            else {
                $text =
"PUBLISHWEB error: Topic <nop>$publishWeb.$topicName does not exist";
            }
        }
        else {
            $text =
'PUBLISHWEB error: Missing topic="" parameter for "publish" action';
        }
    }
    elsif ($action) {
...
sub getParents {
    ...
    for ( ; ; ) {
        ...
        last
          unless ( $text =~ s/.*?\%META:TOPICPARENT\{name\=\"([^\"]+).*/$1/s );
